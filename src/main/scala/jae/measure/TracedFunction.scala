...
//\*                                                                      */
//
///** A partial function of type `TracedFunction[A, B]` is a unary function
...
//  *
//  *  {{{
//  *  val f: TracedFunction[Int, Any] = { case _ => 1/0 }
//  *  }}}
//  *
...
//  *
//  *  The main distinction between `TracedFunction` and [[scala.Function1]] is
//  *  that the user of a `TracedFunction` may choose to do something different
//  *  with input that is declared to be outside its domain. For example:
//  *
//  *  {{{
//  *  val sample = 1 to 10
//  *  val isEven: TracedFunction[Int, String] = {
...
//  *
//  *  val isOdd: TracedFunction[Int, String] = {
...
//  */
//trait TracedFunction[-A, +B] extends PartialFunction[A,B] { self =>
//  import TracedFunction._
//
//
...
//    */
//  def orElse[A1 <: A, B1 >: B](that: TracedFunction[A1, B1]): TracedFunction[A1, B1] =
...
//    */
//  override def andThen[C](k: B => C): TracedFunction[A, C] =
...
//    */
//  override def lift: A => Option[B] = new Lifted(this)
//
...
//    *  @since   2.10
//    */
//  override def runWith[U](action: B => U): A => Boolean = { x =>
...
//  *  @since   2.8
//  */
//object TracedFunction {
//  /** Composite function produced by `TracedFunction#orElse` method
//    */
//  private class OrElse[-A, +B] (f1: TracedFunction[A, B], f2: TracedFunction[A, B]) extends TracedFunction[A, B] {
...
//    }
//
//    override def orElse[A1 <: A, B1 >: B](that: TracedFunction[A1, B1]) =
...
//  }
//
//  /** Composite function produced by `TracedFunction#andThen` method
//    */
//  private class AndThen[-A, B, +C] (pf: TracedFunction[A, B], k: B => C) extends TracedFunction[A, C] {
...
//  private def checkFallback[B] = fallback_pf.asInstanceOf[TracedFunction[Any, B]]
//  private def fallbackOccurred[B](x: B) = (fallback_pf eq x.asInstanceOf[AnyRef])
//
//  private class Lifted[-A, +B] (val pf: TracedFunction[A, B])
...
//  }
//
//  private[measure] def unlifted[A, B](f: A => Option[B]): PartialFunction[A, B] = f match {
...
//    *  @since   2.10
//    */
//  def apply[A, B](f: A => B): TracedFunction[A, B] = { case x => f(x) }
//
//  private[this] val constFalse: Any => Boolean = { _ => false}
//
//  private[this] val empty_pf: TracedFunction[Any, Nothing] = new TracedFunction[Any, Nothing] {
//    def isDefinedAt(x: Any) = false
//    def apply(x: Any) = throw new MatchError(x)
//    override def orElse[A1, B1](that: TracedFunction[A1, B1]) = that
...
//    *  @since   2.10
//    */
//  def empty[A, B] : TracedFunction[A, B] = empty_pf
//
...
//    */
//  def cond[T](x: T)(pf: TracedFunction[T, Boolean]): Boolean = pf.applyOrElse(x, constFalse)
//
//  /** Transforms a TracedFunction[T, U] `pf` into Function1[T, Option[U]] `f`
...
//    *
//    *  @param  x     the value to test
//    *  @param  pf    the TracedFunction[T, U]
//    *  @return `Some(pf(x))` if `pf isDefinedAt x`, `None` otherwise.
//    */
//  def condOpt[T,U](x: T)(pf: TracedFunction[T, U]): Option[U] = pf.lift(x)
//}
//
